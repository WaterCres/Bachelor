...
def results(request):
	...
	if request.method == 'POST':
		...
		# table showing workload of advisors
		advisors = []
		adv = Teacher.objects.all()
		topic_count_g = {}
		topic_count_s = {}
		# count groups pr. topic
		for entry in context['table']:
			key = entry['topic']
			if key in topic_count_g.keys():
				topic_count_g[key] += 1
			else:
				topic_count_g[key] = 1

		# count students pr. topic
		for entry in context['table_expanded']:
			key = entry['topic']
			if key in topic_count_s.keys():
				topic_count_s[key] += 1
			else:
				topic_count_s[key] = 1
		...
		# print(topic_count_s)
		# for teach in adv:
		# 	u_name = teach.user.u_name
		# 	topics = Topic.objects.filter(advisor_main = u_name)
		# 	if topics:
		# 		g_max = []
		# 		g_min = []
		# 		grps = []
		...
		# 		for t in topics:
		# 			g_max += [t.cap_max]
		# 			grps += [topic_count_g[t.title]]
		# 			stds += [topic_count_s[t.title]]
		# 		advisors += [{'username':u_name,
		#   					  'full name': teach.user.full_name,
		# 					  'groups min':sum([t.cap_min for t in topics]),
		# 					  'groups max':g_max,
		# 					  'assigned groups':sum([grps for grps topic_]}]

			
		# 	for entry in assignments:
		# 		if u_name == entry['topic'].advisor_main:
		# 			top = entry['topic']
		# 			advisors += [{  'username': u_name,
		# 							'fullname':top.advisor_main,
		# 							'groups min': top.cap_min,
		# 							'groups max':top.cap_max,
		# 							'assigned groups':}]
